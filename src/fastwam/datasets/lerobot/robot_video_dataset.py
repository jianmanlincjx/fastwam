...
class RobotVideoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # Stage 1 of the goal-pose prior never looks at pixels, so decoding them is
        # pure I/O cost. A placeholder of the right rank is returned instead.
    ...
    def _get(self, idx):
        ...
        vision_free_short_circuit = self.vision_free
        video = None if vision_free_short_circuit else sample["pixel_values"]
        if vision_free_short_circuit:
            # Nothing was decoded, so skip camera composition and resizing entirely and
            # emit the final [C, T_video, H, W] layout directly. Stage 1 skips the VAE and
            # the video expert, so this tensor is only ever shape-checked.
            video = self._vision_free_placeholder()
            # Nothing was decoded, so nothing is padded. Size it by the sampled video
            # frames -- the upstream fallback borrowed the action horizon, which is 32
            # against the 33 frames these indices address.
            ...
            elif not self.vision_free:
                video = video.squeeze(0)  # [T_video, C, H, W]

            if self.vision_free:
                pass  # already [C, T_video, H, W]
            else:
    ...
    def _vision_free_placeholder(self):
        """Final [C, T_video, H, W] layout; T % 4 == 1 and H/W % 16 == 0 as required."""
        return torch.zeros(3, len(self.video_sample_indices), 16, 16)
